...
int main(int argc, char **argv) {
    ...
    unsigned char query[40];
    unsigned char *queries = NULL;
    unsigned int occurrences[16];
    ...
    if (rank==0){
        ...
        queries = Utility::getQueryBuffer();
    }
    
    MPI_Scatter(queries, 40, MPI_UNSIGNED_CHAR, query, 40, MPI_UNSIGNED_CHAR,
                0, MPI_COMM_WORLD);
    ...
    int occurrence = count_occurrences(query, queryLength, document, DOCUMENT_SIZE);
    
    MPI_Gather(&occurrence, 1, MPI_INT, occurrences, 1, MPI_INT, 0, MPI_COMM_WORLD);
